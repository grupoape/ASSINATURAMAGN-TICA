...
// V70 — Recuperação segura: trava scroll do fundo quando modal abre, sem alterar atendimento
function v70AnyModalOpen(){
  return Array.from(document.querySelectorAll('.modal')).some(m => !m.classList.contains('hidden'));
}
function v70SyncBodyModalLock(){
  document.body.classList.toggle('modal-open', v70AnyModalOpen());
}
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.modal').forEach(modal => {
    const obs = new MutationObserver(v70SyncBodyModalLock);
    obs.observe(modal,{attributes:true,attributeFilter:['class']});
    modal.addEventListener('wheel', e => {
      const card = modal.querySelector('.modal-card, .ticket-chat-card');
      if(card && !card.contains(e.target)) e.preventDefault();
    }, {passive:false});
  });
  document.querySelectorAll('.close').forEach(btn => {
    btn.addEventListener('click', () => setTimeout(v70SyncBodyModalLock, 50));
  });
  document.querySelectorAll('.modal').forEach(modal => {
    modal.addEventListener('click', e => {
      if(e.target === modal) setTimeout(v70SyncBodyModalLock, 50);
    });
  });
});
